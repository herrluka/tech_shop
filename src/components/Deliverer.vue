    ...
    <div>
      <Modal modalClass="modal-wrapper" v-model="showDeleteModal">
        <h4 class="modal-title">
          Brisanje dostavljača: {{ itemForDelete.companyName }}
        </h4>
      ...
      <Modal v-model="showUpdateModal" modalClass="modal-wrapper modal-info">
        ...
        <div class="modal-content">
          <p class="text">Naziv kompanije</p>
          <input
            class="modal-input"
            type="text"
            :maxlength="25"
            v-model.lazy="itemForUpdate.companyName"
          />
      ...
      <Modal v-model="showAddModal" modalClass="modal-wrapper">
        <h3 class="modal-title">Dodaj dostavljača</h3>
        <div class="modal-content">
          <p class="text">Naziv kompanije</p>
          <input
            type="text"
            ...
            v-model="newDeliverer.companyName"
          />
          <p class="text">Kontakt</p>
          <input
            type="text"
          ...
          <input
            ...
            v-model="newDeliverer.address"
          />
        </div>
        <div class="delete-action-buttons">
          <button @click="confirmAdd(newDeliverer)" class="btn btn-primary">
            Dodaj
          </button>
...
export default {
  ...
  methods: {
    ...
    openAddDialog() {
      this.newDeliverer.companyName = '';
      this.newDeliverer.address = ''
      ;this.newDeliverer.contact = 0;
    ...
    confirmDelete(id) {
      this.$store.dispatch("deleteDeliverer", id);
      const newDeliverers = this.deliverers.filter(deliverer => {
          return deliverer.id !== id;
        });
      this.$store.commit('setDeliverers', newDeliverers);
    ...
    confirmUpdate() {
      this.$store.dispatch("updateDeliverer", this.itemForUpdate);
      this.showUpdateModal = false;
    },
    confirmAdd(deliverer) {
      const delivererForInsert = {
        companyName: deliverer.companyName,
        address: deliverer.address,
        contact: deliverer.contact,
      };
      this.$store.dispatch("insertDeliverer", delivererForInsert);
